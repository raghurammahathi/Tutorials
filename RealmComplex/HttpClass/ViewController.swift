...
extension ViewController: UITableViewDelegate,UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        blockCell.block = blocks[indexPath.row].data[indexPath.row]
        return blockCell
    }
...
class BlockCell: UITableViewCell {
    ...
    var block:DataModel?{
        didSet {
            if let object = block {
                self.lblBlock.text = object.firstName
                self.lblId.text = object.lastName
                self.lblNew.text = "id is \(object.id.value!)"
